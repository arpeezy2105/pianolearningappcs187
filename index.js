...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const finalHitText = document.getElementById('final-hit');
    const finalMissText = document.getElementById('final-miss');
    const confirmBtn = document.getElementById('confirm-btn');

    function showGameOverPanel(score, hit, miss) {
        if(finalScoreText) finalScoreText.textContent = score + "%";
        if(finalHitText) finalHitText.textContent = hit;
        if(finalMissText) finalMissText.textContent = miss;
...
const noteOrder = [
    ...
    const RHYTHM_SPAWN_INTERVAL_BASE = 600; 
    const RHYTHM_FALL_DURATION = 2500;   
    ...
    function rhythmLoop(time) {
        ...
        if (rhythmSpawnIndex < rhythmQueue.length || anyActive) {
            ...
            setTimeout(() => {
                showGameOverPanel(pct, rhythmHits, rhythmMisses);
            }, 150);
        }
